...
import { ArrowLeft, AlertTriangle, FileText, Trash2, ArrowRight, ClipboardCheck, Info, Eye, EyeOff } from 'lucide-react';
      ...
      <div className="py-8 space-y-8 animate-fade-in">
        <div className="flex flex-col md:flex-row justify-between gap-6">
          ...
          <div className="flex gap-3 h-fit">
            <Button variant="outline" className="text-destructive" onClick={() => deleteAudit(audit.id).then(() => navigate('/history'))}><Trash2 className="h-5 w-5" /></Button>
            <Button asChild className="rounded-xl shadow-lg shadow-primary/20"><Link to="/letters" state={{ audit }}>Dispute Bill <ArrowRight className="ml-2 h-4 w-4" /></Link></Button>
          </div>
        </div>
                ...
                <Card className="rounded-2xl">
                  ...
                  <CardContent className="space-y-6">
                    <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2"><p className="text-xs font-bold uppercase text-muted-foreground">CPT (Procedures)</p>
                        <div className="flex flex-wrap gap-2">{audit.detectedCpt.map(c => <Badge key={c} className="bg-blue-100 text-blue-800 hover:bg-blue-100">{c}</Badge>)}</div>
                      </div>
                      <div className="space-y-2"><p className="text-xs font-bold uppercase text-muted-foreground">ICD-10 (Diagnoses)</p>
                        <div className="flex flex-wrap gap-2">{audit.detectedIcd.map(c => <Badge key={c} variant="outline" className="border-indigo-200">{c}</Badge>)}</div>
                      </div>
                      {audit.detectedRevenue && audit.detectedRevenue.length > 0 && (
                        <div className="space-y-2"><p className="text-xs font-bold uppercase text-muted-foreground">Revenue Codes</p>
                          <div className="flex flex-wrap gap-2">{audit.detectedRevenue.map(c => <Badge key={c} className="bg-orange-100 text-orange-800 hover:bg-orange-100">{c}</Badge>)}</div>
                        </div>
                      )}
                    </div>
                  </CardContent>
                  ...
                  <CardContent className="space-y-4">
                    {audit.flags.map((f, i) => (
                      <div key={i} className="p-3 bg-white/60 border border-amber-200 rounded-xl">
                        <div className="flex justify-between items-start mb-1"><p className="text-xs font-bold uppercase text-amber-900">{f.type}</p><Badge className="scale-75 origin-right">{f.severity}</Badge></div>
